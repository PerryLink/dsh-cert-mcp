...
- Node `^22.19.0 || >=24.0.0`.
- DSH `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0 || >=0.1.6-0 <0.2.0 || >=0.1.7-0 <0.2.0` (declarado en `engines.dsh`, con `dsh.manifestVersion: 1`).
...
```sh
pnpm install
pnpm run typecheck                            # la regla publicada: checkJs sobre la cara host instalada
pnpm run typecheck:checkout                   # la regla del checkout: el mismo conjunto de archivos contra los tipos construidos del checkout local del harness
